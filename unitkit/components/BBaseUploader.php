 ...
 * @version 1.0
 */
class BBaseUploader extends CComponent
{
    ...
    {
        return B::t('unitkit', 'uploader_max_size') . ': ' .
    ...
    public function saveTmpFile($modelName, $attributeName, $overview = array(), $cache = 86400 /* 24h */)
    {
        ...
        if ($model->validate(array($attributeName))) {
            // generate a key
            $tmpKey = BTools::sha256(uniqid(mt_rand(), true) . ':' . Yii::app()->session->sessionID);
            // generate a file name
            $nameFile = mb_strtolower(sha1(uniqid(mt_rand(), true)) . '.' . $model->$attributeName->getExtensionName(), 'UTF-8');
            // data
            $data = array(
                'file' => $nameFile,
                'id' => BTools::sha512(Yii::app()->session->sessionID),
                'overview' => $overview
            );
        ...
        switch ($overview['type']) {
            case self::OVERVIEW_IMAGE:
                $url = Yii::app()->controller->createAbsoluteUrl($overview['route'], array(
                    'overview' => BHtml::activeName($model, $attribute),
                    'key' => $tmpKey
                ));
                $html = BHtml::image($url, '', $overview['html_options']);
                break;
        }
        ...
        switch ($overviewOptions['type']) {
            case self::OVERVIEW_IMAGE:
                if (! empty($command) && $command['action'] === self::ACTION_INSERT) {
                    ...
                    $html .= BHtml::image($uploadVar[$attribute]['urlDest'] . '/' . $model->$attribute, '', $overviewOptions['html_options']);
                }
                break;
            default:
                $html .= '';
    ...
    public function htmlUploader(&$model, $attribute, $uploaderUrl, $overviewOptions = array())
    {
		...
		<div class="upload-file">
			<a href="#" class="btn btn-default btn-upload" ' . 'data-model="' . BHtml::activeName($model, $attribute) . '" ' . 'data-action="' . $uploaderUrl . '" ' . 'data-size="' . $this->getDataSize($model, $attribute) . '" ' . 'data-type="' . $this->getDataTypes($model, $attribute) . '" ' . 'data-sessKey="' . Yii::app()->session['sess_key'] . '" ' . 'data-overview="' . htmlentities(json_encode($overviewOptions)) . '" ' . '>
				<span class="glyphicon glyphicon-upload"></span>
				<span>' . B::t('unitkit', 'btn_upload') . '</span>
			</a>
        ...
        if (! empty($model->$attribute)) {
            ...
            if (! empty($command) && $command['action'] === self::ACTION_INSERT &&
                ...
                $html .= '
					<div class="upload-file-progress original" style="display:none;">
						...
						$this->htmlOverview($oldModel, $attribute, $overviewOptions) . BHtml::activeHiddenField($oldModel, $attribute, array(
            ...
            $html .= '
				<div class="upload-file-progress ' . $cssClass . '" style="' . (! empty($command) && $command['action'] === self::ACTION_DELETE ? 'display:none;' : '') . '">
					...
					' . $this->htmlOverview($model, $attribute, $overviewOptions) . BHtml::activeHiddenField($model, $attribute, array(
                'class' => 'upload-file-input',
                'id' => false,
                'value' => $hiddenValue
            )) . '
