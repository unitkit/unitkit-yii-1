 ...
 * @version 1.0
 */
abstract class UBaseAutoController extends UBaseController
{
    ...
    public function actionAdvComboBox()
    {
        if (isset($_GET['name'])) {
            ...
            if (! empty($data[$_GET['name']])) {
                echo CJSON::encode(UHtml::listDataAdvCombobox($data[$_GET['name']]['model'], $data[$_GET['name']]['select'], $data[$_GET['name']]['search'], $data[$_GET['name']]['criteria'], $data[$_GET['name']]['cache']));
            }
        }
        Yii::app()->end();
    }
    ...
    public function actionExport()
    {
        ...
        // generate csv export
        $export = new UActiveRecordExportCsv($this->_model, $dataProvider->getData());
        $export->download();
    }
            ...
            if (! empty($model::$upload)) {
                foreach ($_POST['rows'] as $tmp) {
                    ...
                    foreach ($model::$upload as $column => $data) {
                        if($record->$column !== '') {
                            $uploader = $this->getUploader(UHtml::resolveName($model, $column));
                            $record->uploadOperations[$column]['action'] = UUploader::ACTION_DELETE;
