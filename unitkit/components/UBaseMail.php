 ...
 * @version 1.0
 */
abstract class UBaseMail extends CComponent
{
     ...
     */
    public $classFunction = 'UMailFunction';
    ...
    public function sendMailTemplate($id, $i18nId)
    {
        ...
        if ($mailTemplate === null) {
            throw new CException(Unitkit::t('unitkit', 'mail_template_not_exist'));
        } elseif (! isset($mailTemplate->uMailTemplateI18ns[0])) {
            throw new CException(Unitkit::t('unitkit', 'mail_template_not_translated'));
        }

        // get sending roles
        $modelSendingRoles = new UMailSendingRole();
        ...
        $sendingRoles = $modelSendingRoles->with(array(
            ...
            'u_mail_template_id' => $mailTemplate->id
        ));

        if (count($sendingRoles) == 0) {
            throw new Exception(Unitkit::t('unitkit', 'mail_no_sending_role'));
        }

        // decode params
        $mailObject = $mailTemplate->uMailTemplateI18ns[0]->object;
        $mailBody = $mailTemplate->uMailTemplateI18ns[0]->message;
        ...
        if ($mailTemplate->sql_request != '') {
            ...
            if ($resQuery === false) {
                throw new Exception(Unitkit::t('unitkit', 'mail_template_request_error'));
            }
        ...
        foreach ($sendingRoles as $sendingRole) {
            switch ($sendingRole->u_mail_send_role_id) {
                case Unitkit::v('unitkit', 'u_send_role_id:from'):
                    $this->mailer->SetFrom($sendingRole->uPerson->email, isset($this->fromName) ? utf8_decode($this->fromName) : '');
                    $this->mailer->Sender = $sendingRole->uPerson->email;
                    break;
                case Unitkit::v('unitkit', 'u_send_role_id:to'):
                    $this->mailer->AddAddress($sendingRole->uPerson->email);
                    break;
                case Unitkit::v('unitkit', 'u_send_role_id:cc'):
                    $this->mailer->AddCC($sendingRole->uPerson->email);
                    break;
                case Unitkit::v('unitkit', 'u_send_role_id:bcc'):
                    $this->mailer->AddBCC($sendingRole->uPerson->email);
                    break;
            }
        }
