...
class UBaseDbMessageSource extends CDbMessageSource
{
    ...
    public function refreshCache()
    {
        ...
        $groups = UMessageGroup::model()->findAll(array(
            'select' => 'id'
        ));
...
FROM
	u_message t1, u_message_i18n t2
WHERE
	t1.id = t2.u_message_id AND
	t1.u_message_group_id = :u_message_group_id AND
	t2.i18n_id = :language
EOD;
        $command = UMessage::model()->dbConnection->createCommand($sql);
        $command->bindValue(':u_message_group_id', $group);
