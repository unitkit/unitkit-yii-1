...
class UActiveRecordExportCsv
{
    ...
    protected function buildHeader()
    {
        if (! empty($this->_header)) {
            foreach ($this->_header as $attribute) {
                $this->_rows[0][] = UTools::utf8Decode($this->_modelClass->getAttributeLabel($attribute));
            }
        } else {
            foreach ($this->_modelClass->attributeLabels() as $attr => $label) {
                $this->_rows[0][] = UTools::utf8Decode($label);
                $this->_header[] = $attr;
            }
        }
    }
    ...
    protected function buildContent()
    {
        foreach ($this->_models as $index => $model) {
            $i = $index + 1;
            foreach ($this->_header as $attribute) {
                $this->_rows[$i][] = UTools::utf8Decode($this->getAttributeValue($attribute, $index));
            }
        }
    }
