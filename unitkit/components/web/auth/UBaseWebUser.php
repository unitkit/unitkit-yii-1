 ...
 * @version 1.0
 */
class UBaseWebUser extends CWebUser
{
    /**
     * List of roles
     *
    ...
    protected function restoreFromCookie()
    {
        ...
        if ($cookie && ! empty($cookie->value) && is_string($cookie->value) && ($data = $app->getSecurityManager()->validateData($cookie->value)) !== false) {
            ...
            if (is_array($data) && isset($data[0 /* uuid */], $data[1 /* key1 */], $data[2 /* key2 */])) {
                $identity = new UUserIdentityAuto($data[0 /* uuid */], $data[1 /* key1 */], $data[2 /* key2 */]);
    ...
    public function login($identity, $duration = 0)
    {
        if (Yii::app()->rights->hasRole($identity->getId(), $this->_navigateRole)) {
            ...
            ULanguagesApp::setLanguage($this->getPerson()->default_language);
    ...
    public function logout($destroySession = true)
    {
        if ($this->allowAutoLogin) {
            ...
            $uuid = $this->getState('uAutoLogin:uuid');
            if ($uuid !== null) {
                UAutoLogin::model()->deleteAllByAttributes(array(
                    'uuid' => $uuid
                ));
            }
        }
    ...
    {
        $model = new UAutoLogin();

        // begin transaction
        $transaction = $model->dbConnection->beginTransaction();
        try {
            // generate uuid and keys
            $this->_autoLoginData[0 /* uuid */] = UTools::sha256(uniqid(mt_rand(), true) . ':' . UTools::password(500));
            $this->_autoLoginData[1 /* key 1 */] = UTools::sha512(uniqid(mt_rand(), true) . ':' . UTools::password(500));
            $this->_autoLoginData[2 /* key 2 */] = UTools::sha512(uniqid(mt_rand(), true) . ':' . UTools::password(500));
            ...
            $model->u_person_id = $this->getId();
            $model->duration = $duration;
            $model->created_at = UTools::now();
            ...
            if ($model->save()) {
                ...
                $this->setState('uAutoLogin:uuid', $this->_autoLoginData[0 /* uuid */]);
