 ...
 * @version 1.0
 */
class UUserIdentityAuto extends UBaseUserIdentity
{
    /**
     * User id
     *
    ...
    {
        $model = UPerson::model()->getByUUID($this->_uuid);

        if ($model === null || ! CPasswordHelper::verifyPassword($this->_key1, $model->uAutoLogins[0]->key1) || ! CPasswordHelper::verifyPassword($this->_key2, $model->uAutoLogins[0]->key2)) {
        ...
        elseif ($model->validated == 0)
            ...
            $this->_id = $model->id;
            $this->_model = $model->uAutoLogins[0];
