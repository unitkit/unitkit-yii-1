    ...
    <div class="col-md-6">
        ...
        <?php else: ?>
        	...
        	<div class="alert alert-danger">
        		<?= UHtml::errorSummary($models['NewPasswordForm']); ?>
        	</div>
        	<?php endif; ?>
            <form method="POST" class="csrf" action="<?= $this->createUrl($this->id.'/validate', array('code' => $_GET['code'], 'uuid' => $_GET['uuid'])); ?>">
                <div class="form-group <?= $models['NewPasswordForm']->hasErrors('password') ? 'error' : ''; ?>">
                    ...
                    <?= UHtml::activePasswordField($models['NewPasswordForm'], 'password', array('class' => 'form-control input-sm')); ?>
                </div>
                <div class="form-group <?= $models['NewPasswordForm']->hasErrors('repeat_password') ? 'has-error' : ''; ?>">
                    ...
                    <?=UHtml::activePasswordField($models['NewPasswordForm'], 'repeat_password', array('class' => 'form-control input-sm'));?>
                </div>
                <?=UHtml::button(Unitkit::t('unitkit', 'btn_save'), array('type' => 'submit','class' => 'btn btn-primary'));?>
            </form>
        <?php endif; ?>
        </div>
    </div>
