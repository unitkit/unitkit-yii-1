...
class BNavBar extends CWidget
{
    ...
    protected function buildItem(&$item)
    {
        ...
        if (! isset($item['label']['url']))
            return BHtml::tag('span', $item['label']['htmlOptions'], $item['label']['value']);

        if (isset($item['label']['hasRight']) && ! $item['label']['hasRight'])
            $item['label']['url'] = '#';

        return BHtml::link($item['label']['value'], $item['label']['url'], $item['label']['htmlOptions']);
    }
    ...
    protected function buildItems($items, $level = 0)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($itemHtml != '')
                $html .= BHtml::tag('li', $item['htmlOptions'], $itemHtml);
        }
